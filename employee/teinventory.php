    ...
    if(isset($_POST['add'])){
        ...
        if($result_i) {
            ...
            if($supp_row['ProductType']=="Medicine"){
                ...
                $sql_inser="INSERT into Tests(InventoryID,TestType,TestKit) values ('$Inventoryid','$product','$kit')";
                $sq_result=mysqli_query($conn,$sql_inser);
                if($sq_result){
                    header("Location:employee_inventory.php?success=newinventoryadded&sid={$Inventoryid}");        
                    
                }
                else{
                    header("Location:teinventory.php?mnosuccess&EID=".$Inventoryid. "&InventoryName=" .$Inventory_name."&ED=" .$Expiry_date."&SID=".$Suppliers_id."&NI=".$Number_items. "&Price=".$price."&Type=".$product."&kit=".$kit."&SN=".$SN); 
                }
            }
                
        }
        else{
            header("Location:teinventory.php?nosuccess&EID=".$Inventoryid. "&InventoryName=" .$Inventory_name."&ED=" .$Expiry_date."&SID=".$Suppliers_id."&NI=".$Number_items. "&Price=".$price."&Type=".$product."&kit=".$kit."&SN=".$SN); 
        }
       }
}
...
<div class="card-header" style="width:70%;background:rgb(6, 24, 9)">
  <h5 class=" info-color white-text text-center py-4" style="color:white">
    <strong>NEW TEST KIT DETAILS</strong>
  </h5>
  </div>
...
<option  value='<?php if(isset($_GET['kit'])){echo $_GET['kit'];}?>'><?php if(isset($_GET['kit'])){echo $_GET['kit'];}?></option>
