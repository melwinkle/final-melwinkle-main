...
$lname=$rowname['EmployeeLname'];
    ...
    $sales_query="SELECT sum(Bill) as Total_sales from Sellsto where EmployeeID='$id' and EXTRACT(YEAR from DateofSale)= year(curdate())";
    ...
    $bill="SELECT sum(Bill) as Month_Bills from Sellsto  where EmployeeID='$id' and EXTRACT(MONTH from DateofSale)=month(curdate()) and EXTRACT(YEAR from DateofSale)= year(curdate())";
    ...
    $inventor=mysqli_num_rows($invent);
      ...
      $sql_bill="SELECT sum(Bill) as Total_Sale FROM Administers where EmployeeID='$id' and EXTRACT(YEAR from DateAdministered)= year(curdate())";
      ...
      $billz="SELECT sum(Bill) as Month_Bill from Administers  where EmployeeID='$id' and  EXTRACT(MONTH from DateAdministered)=month(curdate()) and EXTRACT(YEAR from DateAdministered)= year(curdate())";
...
<ul class="list-items">
  ...
  <li><a href='/final-melwinkle/employee/employee_dashboard.php'><i class='icon ion-md-home' size='large'></i> Dashboard</a></li>
 ...
 <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                        <div class="white-box" style="background:white;padding:25px;margin-bottom:15px">
                            <div class="col-in row">
                                <div class="col-md-6 col-sm-6 col-xs-6"> <i data-icon="E"
                                        class="linea-icon linea-basic"></i>
                                    <h5 class="text-muted vb">SALARY(GHC)</h5>
                                </div>
                                <div class="col-md-6 col-sm-6 col-xs-6">
               ...
               <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
                  <div class="white-box" style="background:white;padding:25px;margin-bottom:15px">
                      <div class="col-in row">
                          ...
                          <div class="col-md-6 col-sm-6 col-xs-6">
                              <h3 class="counter text-right m-t-15 text-danger"><?php
                                    $sql_sll="SELECT * FROM Inventory where NumberofItems<100";
                                    $res_l=mysqli_query($conn,$sql_sll);
                                    $rl=mysqli_num_rows($res_l);
...
<div class="row" style="margin-top:3%;margin-left:5%;padding-right:5px">              
          ...
          $sql="SELECT * FROM Employee inner join Employee_Log_in ON Employee.EmployeeID=Employee_Log_in.EmployeeID WHERE Employee.EmployeeID='$id'";
               ...
               <button type='button' class='btn btn-primary' data-toggle='modal' data-target='#requestleaves_<?php echo $id;?>'>
                   REQUEST LEAVE<i class='icon ion-md-checkmark-circle-outline' size='large'></i>
                 </button>
               <button type='button' class='btn btn-primary' data-toggle='modal' data-target='#requests_<?php echo $id;?>'>
                   UPDATE<i class='icon ion-md-checkmark-circle-outline' size='large'></i>
                 </button>
                     
         
                     <?php include_once('update_e.php')?>
                     </div>
           </form>
